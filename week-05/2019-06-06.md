...
## Head and Body
The HTML we've written so far is working just fine, but it is not complete. Let's take a look at a properly formated HTML page. Replace your code in `index.html` with the code below. 
...
### div
A `<div></div>` tag is a general container. It is used to encapsilate other elements and to break your page up into sections. 
...
CSS is the language we use to handle the style and layout of our web pages. We apply CSS rules to HTML elements by first using a **selector** to target an object or group of objects. Then, we can set properties on the elmenet/elements we have selected. 

## Adding Styles

You can apply CSS directly to the element, but this is usually discouraged. More commonly we write CSS in a seperate file that we then link to in the `<head>` tag. Create a new file in the same directory as your `index.html` called `styles.css`. Then put a link in the head of your HTML file. 
...
```
### CSS Grid 
Positioning elements with CSS used to be very difficult. [The old way](https://www.w3schools.com/css/css_positioning.asp) relied heavily on positioning elements releative to other elments. Eventually [flex box](https://www.w3schools.com/Css/css3_flexbox.asp) came along as a way to break pages up into columns. Since then, [CSS Grid](https://css-tricks.com/snippets/css/complete-guide-grid/) has emereged as the best way to position elements on the page. CSS Grid allows you to break elements up into columns and rows. Let's replace everything in our `index.html` file with the code below. 
