...
At your host prompt (not your container prompt), run this command:
```
Mac:
docker run -it --name server8000 -p 8000:8000 -v $(pwd):/usr/local/src/code_platoon -v $(pwd)/.bashrc:/root/.bashrc codeplatoon/base

Windows:
docker run -it --name server8000 -p 8000:8000 -v ${pwd}:/usr/local/src/code_platoon -v ${pwd}/.bashrc:/root/.bashrc codeplatoon/base
```

Now, when you want to use port 8000, you can use `docker start -i server8000` instead of `docker start -i codePlatoon`
...
### Models & Migrations
Django's architecture is based off the MVC framework - Model, View, Controller. The Controller layer can be seen as the brains of the app; it handles the logic (this is views.py file) . The View layer is responsible for everything that a user sees (the Templates with HTML), and the Model layer is what connects your Python objects to the database (the models.py file).
...
- [Posts and Comments](https://github.com/kiloplatoon/posts-and-comments)
