...
urlpatterns = [
    path('admin/', admin.site.urls),
    path('accounts/', include('django.contrib.auth.urls')),
    path('', TemplateView.as_view(template_name='registration/home.html'), name='home'), # new
]
```

Next, in the `templates/registration/` folder you created, create two additional files. First, `base.html`:
