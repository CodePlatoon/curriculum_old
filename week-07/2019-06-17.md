...
**Don't worry about any unapplied migrations yet. Where not using our database just yet** 
...
### Templating
Of course, having plain text is not terribly useful. We want to create templates in HTML (the View layer of MVC) where we can pass in Python objects of data. Under your `polls` directory, create a `templates` folder, and under that directory, create a `polls` folder, and under that directory create a `index.html` file:
