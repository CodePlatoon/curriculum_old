...
Finally, let's get our models going in `cohorts_and_students/models.py`:
...
class Student(models.Model):
    ...
    first_name = choice_text = models.CharField(max_length=200)
    last_name = choice_text = models.CharField(max_length=200)
...
def new_cohort(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            cohort.save()
            return redirect('cohort_detail', cohort_id=cohort.pk)
    else:
        form = CohortForm()
...
def edit_cohort(request, cohort_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            cohort = form.save(commit=False)
            cohort.save()
            return redirect('cohorts/cohort_detail', cohort_id=cohort.pk)
    else:
...
Just like that, we can CRUD one resource. This is stuff we already know. Let's do a nested CRUD in the next section.
...
```
Next, let's add those controller actions:
```python
