...
Now, let's clean it up a little bit. If we have a lot of props getting passed down, it gets ugly if we have to write `this.props` so many times. Because this.props is a javascript object, we can declare new variables like this:
...
class MuteButton extends React.Component {
  render() {
    ...
    if (isMuted === true) {
      return (
        <div onClick={toggleMute}> <img src={OffIcon} /> </div>
      )
    }

    return (
      <div onClick={toggleMute}> <img src={OnIcon} /> </div>
    )
  }
}
...
export default MuteButton;
```
There, that's easier to read! You will eventually want to be able to write both class-based and functional components; for now you can just stick with one or the other if you like. Below is what the code would look like with functional components:
