...
Let's install `create-react-app` globally by running `npm install -g create-react-app` and run `create-react-app react-example`. Wow, it's very similar to Django in that there's a lot of stuff going on. Let's break each one down:
