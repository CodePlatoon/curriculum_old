...
def landmark_list(request):
    landmarks = landmark.objects.order_by('published_date')
...
```html
<div>
  <h1><a href="/map/landmarks">My map</a></h1>
</div>

{% for landmark in landmarks %}
  <div>
      <h2><a href="">{{ landmark.title }}</a></h2>
      <p>published: {{ landmark.published_date }}</p>
      <p>{{ landmark.text|linebreaksbr }}</p>
  </div>
{% endfor %}
```
...
```
Create a `landmark/post_detail.html` template:
```html
<div class="landmark">
    <h2>{{ landmark.name }}</h2>
