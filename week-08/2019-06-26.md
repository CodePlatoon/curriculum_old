...
```
<button onClick={this.handleButtonClick}> I've been clicked {this.state.clickCount} times</button>
```
  ...
  render() {
    return (
      <div className="App">
        ...
        <p className="App-intro">
          <button onClick={this.handleButtonClick}> I have been clicked {this.state.clickCount} times </button>
        </p>
      </div>
    );
  }
...
Since `render` is bound to the `class App` and we're calling a function within a function (`handleButtonClick` to `render`), `handleButtonClick` is no longer aware of its scope. We can solve this by changing our button code to the following:

```
<button onClick={this.handleButtonClick.bind(this)}> I've been clicked {this.state.clickCount} times</button>
```
...
class ButtonComponent extends Component {
  ...
  render(){
    return (
      <button onClick={this.handleButtonClick.bind(this)}> I have been clicked {this.state.clickCount} times</button>
    )
  }
}
...
Challenges
----------
* [Create React App](https://github.com/indiaplatoon/create-react-app)
